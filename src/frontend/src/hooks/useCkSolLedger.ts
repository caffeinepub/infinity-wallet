...
import { createCkSolLedgerClient, type ICRC1LedgerActor } from '@/lib/ckSolLedger';

/**
 * Hook to get an authenticated ckSOL ledger client with comprehensive debugging
 * 
 * Returns null if the user is not signed in.
 */
export function useCkSolLedger() {
  ...
  useEffect(() => {
    ...
    async function initClient() {
      ...
      console.group(`🔧 [${timestamp}] ckSOL Ledger Client Initialization`);
      ...
      try {
        console.log('🚀 Creating ckSOL Ledger client...');
        const client = await createCkSolLedgerClient(identity);
        ...
        console.error('❌ Failed to initialize ckSOL Ledger client:', {
