...
import { useCkEthLedger } from './useCkEthLedger';
import { getDefaultAccount } from '@/lib/ckEthLedger';
import { useInternetIdentity } from './useInternetIdentity';

/**
 * Hook to fetch ckETH balance from the live ICRC-1 ledger with comprehensive debugging
 */
export function useCkEthBalance() {
  const { ledgerClient, isInitializing, isAuthenticated } = useCkEthLedger();
  const { identity } = useInternetIdentity();

  const query = useQuery<bigint>({
    queryKey: ['ckEthBalance', identity?.getPrincipal().toString()],
    queryFn: async () => {
      const timestamp = new Date().toISOString();
      console.group(`🔍 [${timestamp}] ckETH Balance Fetch`);
