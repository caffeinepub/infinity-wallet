...
import { getDefaultAccount } from '@/lib/ckBtcLedger';
import { useInternetIdentity } from './useInternetIdentity';

/**
 * Hook to fetch ckBTC balance from the live ICRC-1 ledger with comprehensive debugging
 */
export function useCkBtcBalance() {
  const { ledgerClient, isInitializing, isAuthenticated } = useCkBtcLedger();
  const { identity } = useInternetIdentity();

  const query = useQuery<bigint>({
    queryKey: ['ckBtcBalance', identity?.getPrincipal().toString()],
    queryFn: async () => {
      const timestamp = new Date().toISOString();
      console.group(`🔍 [${timestamp}] ckBTC Balance Fetch`);
