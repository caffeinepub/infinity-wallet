...
import { CKETH_LEDGER_CANISTER_ID } from './config';
...
/**
 * Create an authenticated ICRC-1 ckETH Ledger client with comprehensive debugging
 */
export async function createCkEthLedgerClient(
  identity: Identity,
  host?: string
): Promise<ICRC1LedgerActor> {
  const timestamp = new Date().toISOString();
  console.group(`🏗️ [${timestamp}] Creating ckETH Ledger Client`);
  ...
  try {
    ...
    console.log('🎯 Canister Configuration:', {
      ...
      canisterIdLength: CKETH_LEDGER_CANISTER_ID.length,
