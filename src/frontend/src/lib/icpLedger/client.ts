...
/**
 * Create an authenticated ICRC-1 ICP Ledger client with comprehensive debugging
 */
export async function createIcpLedgerClient(
  identity: Identity,
  host?: string
): Promise<ICRC1LedgerActor> {
  const timestamp = new Date().toISOString();
  console.group(`🏗️ [${timestamp}] Creating ICP Ledger Client`);
  ...
  try {
    ...
    console.log('🎯 Canister Configuration:', {
      ...
      canisterIdLength: ICP_LEDGER_CANISTER_ID.length,
