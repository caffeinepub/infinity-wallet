...
import { CKSOL_LEDGER_CANISTER_ID } from './config';
...
/**
 * Create an authenticated ICRC-1 ckSOL Ledger client with comprehensive debugging
 */
export async function createCkSolLedgerClient(
  identity: Identity,
  host?: string
): Promise<ICRC1LedgerActor> {
  const timestamp = new Date().toISOString();
  console.group(`🏗️ [${timestamp}] Creating ckSOL Ledger Client`);
  ...
  try {
    ...
    console.log('🎯 Canister Configuration:', {
      canisterId: CKSOL_LEDGER_CANISTER_ID,
      canisterIdLength: CKSOL_LEDGER_CANISTER_ID.length,
